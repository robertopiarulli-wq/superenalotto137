...
try:
    ...
    # ECCO LO SLIDER REINSERITO
    ampiezza_pool = st.sidebar.slider("Ampiezza Pool Bilanciamento", 12, 30, 20)
    ...
    if st.button("🚀 GENERA ARROSTO AUTOMATIZZATO"):
        ...
        if n_mancanti > 0:
            ...
            # LOGICA DI COMPENSAZIONE: cerchiamo numeri che aiutino a raggiungere la media
            # Usiamo tutto il pool_residuo per avere massima flessibilità
            ...
            st.write(f"🛠️ **Compensazione**: Media necessaria per i restanti: **{int(media_target)}**")
        else:
        ...
        for i, comb in enumerate(combs):
            ...
            if all(f in s for f in cardini):
                ...
                # Controllo fasce sature
                check_satura = any(s_f[0] < somma_s <= s_f[1] for s_f in sature)
                
                if check_valle and not check_satura:
                    h_s = calcola_rugosita(s)
                    if abs(h_s - target_h) < (target_h * 0.10): # Tolleranza leggermente più elastica
                        err = abs(h_s - target_h)
                        sestine_nobili.append((s, err, h_s, somma_s))
            
            if i % 5000 == 0: prog.progress((i+1)/len(combs))
        prog.empty()
        
        if sestine_nobili:
            st.subheader(f"✨ L'Arrosto: {len(sestine_nobili)} Sestine")
            st.table(pd.DataFrame(sorted(sestine_nobili, key=lambda x: x[1])[:30], 
                                 columns=['Sestina', 'Errore', 'Rugosità', 'Somma']))
        else:
            st.error("Nessun incastro trovato. Aumenta l'ampiezza del pool o cambia i cardini per bilanciare meglio la somma.")
